...
//			bCopyFile(szGetFullPathName(_T("ConfigAnalyseur.ini"),szText),SZ_FIC_TEMP,FALSE);
			...
			//Zero
            dwGetPrivateProfileString(_T("Config"), _T("ZeroCyclePresent"),_T("0"),szText, sizeof(szText)/sizeof(TCHAR), szGetFullPathName(_T("ConfigAnalyseur.ini")));
            QString sLigneIni = QString::fromWCharArray(szText);
			QStringList sListLigneIni = sLigneIni.split("|");
			if (sListLigneIni.count() > 1 && sListLigneIni.at(2).toInt())
			{
                dwGetPrivateProfileString(_T("Config"), _T("ZeroCycleLabel"),_T("0"),szText, sizeof(szText)/sizeof(TCHAR), szGetFullPathName(_T("ConfigAnalyseur.ini")));
                sLigneIni = QString::fromWCharArray(szText);
				sListLigneIni = sLigneIni.split("|");
				lblZero->setText(sListLigneIni.at(2));
				layoutCentral->addRow(lblZero, btExecuteZero);
     		}
			//Calibration
            dwGetPrivateProfileString(_T("Config"), _T("CalibrageCyclePresent"),_T("0"),szText, sizeof(szText)/sizeof(TCHAR), szGetFullPathName(_T("ConfigAnalyseur.ini")));
            sLigneIni = QString::fromWCharArray(szText);
			sListLigneIni = sLigneIni.split("|");
			if (sListLigneIni.count() > 1 && sListLigneIni.at(2).toInt())
			{
                dwGetPrivateProfileString(_T("Config"), _T("CalibrageCycleLabel"),_T("0"),szText, sizeof(szText)/sizeof(TCHAR), szGetFullPathName(_T("ConfigAnalyseur.ini")));
                sLigneIni = QString::fromWCharArray(szText);
				sListLigneIni = sLigneIni.split("|");
#ifdef CERIUM
				if(i == 3)
            ...
            dwGetPrivateProfileString(_T("Config"), _T("CalibrageInLineCyclePresent"),_T("0"),szText, sizeof(szText)/sizeof(TCHAR), szGetFullPathName(_T("ConfigAnalyseur.ini")));
            sLigneIni = QString::fromWCharArray(szText);
			sListLigneIni = sLigneIni.split("|");
			if (sListLigneIni.count() > 1 && sListLigneIni.at(2).toInt())
			{
                dwGetPrivateProfileString(_T("Config"), _T("CalibrageInLineCycleLabel"),_T("0"),szText, sizeof(szText)/sizeof(TCHAR), szGetFullPathName(_T("ConfigAnalyseur.ini")));
                sLigneIni = QString::fromWCharArray(szText);
				sListLigneIni = sLigneIni.split("|");
			...
			//Mesure manuel
            dwGetPrivateProfileString(_T("Config"), _T("CleanupCyclePresent"),_T("0"),szText, sizeof(szText)/sizeof(TCHAR), szGetFullPathName(_T("ConfigAnalyseur.ini")));
            sLigneIni = QString::fromWCharArray(szText);
			sListLigneIni = sLigneIni.split("|");
			if (sListLigneIni.count() > 1 && sListLigneIni.at(2).toInt())
			{
                dwGetPrivateProfileString(_T("Config"), _T("CleanupCycleLabel"),_T("0"),szText, sizeof(szText)/sizeof(TCHAR), szGetFullPathName(_T("ConfigAnalyseur.ini")));
                sLigneIni = QString::fromWCharArray(szText);
				sListLigneIni = sLigneIni.split("|");
