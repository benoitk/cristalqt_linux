...
bool CDialogAlarm::updateAlarms()
{
	...
	for(int i=0; i<m_pSupervision->getAnalyseur()->m_ListMessageError.iGetCount();i++)
	{
        ...
        qDebug() << QString::fromWCharArray(m_pSupervision->getAnalyseur()->m_ListMessageError.pGetAt(i)->szGetLabel()
										, m_pSupervision->getAnalyseur()->m_ListMessageError.pGetAt(i)->lGetLabelLength());
        *///QString::fromWCharArray(CElemCycle::m_CurrentStep.szGetLabel()
		//			     , CElemCycle::m_CurrentStep.lGetLabelLength())
        if(QString::fromWCharArray(m_pSupervision->getAnalyseur()->m_ListMessageError.pGetAt(i)->szGetLabel()
											 , m_pSupervision->getAnalyseur()->m_ListMessageError.pGetAt(i)->lGetLabelLength()) 
			!= "CElemBase")
		{		
            qDebug() << QString::fromWCharArray(m_pSupervision->getAnalyseur()->m_ListMessageError.pGetAt(i)->szGetLabel()
            ...
            sTmp = QString::fromWCharArray(m_pSupervision->getAnalyseur()->m_ListMessageError.pGetAt(i)->szGetLabel());
