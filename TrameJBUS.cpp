...
BOOL CElemTrameJbus::bSetConfig(LPTSTR pszText,int &iCurrentPos,CListStream *pListStream)
{
    ...
    pcToken = _tcstok( pszText, szSeps , &state);
	if (pcToken != NULL)
	{
		iCurrentPos += (lstrlen(pcToken) + 1);
		bReturn = m_NumDest.bSetTextVal(pcToken);
        pcToken = _tcstok( NULL, szSeps, &state );
	}
	// code fonction
...
LPTSTR CElemTrameJbus::szGetConfig(LPTSTR pszText, int iSizeMax)
{
	...
	// num d'esclave (destinataire)
//    _stprintf(pszText, sizeof(pszText)/sizeof(TCHAR),_T("%d"),m_NumDest.ucGetVal());
    _stprintf(pszText, iSizeMax,_T("%d"),m_NumDest.ucGetVal());
	lstrcat(pszText,szSeps);
	// code fonction
    _stprintf(szTemp, sizeof(szTemp)/sizeof(TCHAR),_T("%d"),m_CodeFct.ucGetVal());
...
BOOL CElemTrameJbusReadBitsRQ_1_2::bSetConfig(LPTSTR pszText,int &iCurrentPos,CListStream *pListStream)
{
    ...
    _stprintf(szText, sizeof(szText)/sizeof(TCHAR),_T("CElemTrameJbusReadBitsRQ_1_2 : %S"),pszText);
	SetLabel(szText);
...
LPTSTR CElemTrameJbusReadBitsRQ_1_2::szGetConfig(LPTSTR pszText, int iSizeMax)
{
	...
	// adresse
    _stprintf(szTemp, sizeof(szTemp)/sizeof(TCHAR),_T("0x%08x"),m_StartAddr.nGetVal());
	lstrcat(pszText,szTemp);
	lstrcat(pszText,szSeps);
	// nbr bit
    _stprintf(szTemp, sizeof(szTemp)/sizeof(TCHAR),_T("%d"),m_NbrBits.nGetVal());
	lstrcat(pszText,szTemp);
...
BOOL CElemTrameJbusReadBitsRP_1_2::bSetConfig(LPTSTR pszText,int &iCurrentPos,CListStream *pListStream)
{
    ...
    _stprintf(szText, sizeof(szText)/sizeof(TCHAR),_T("CElemTrameJbusReadBitsRP_1_2 : %S"),pszText);
	SetLabel(szText);
	if (bReturn = CElemTrameJbus::bSetConfig(pszText,iCurrentPos,pListStream))
	{
		pszText += iCurrentPos;
					...
					for (j = 0; bReturn && (j < 8); j++)
					{
						if ((pcToken != NULL) && bReturn)
						{
							...
							if ((_tcsstr(pcToken,_T("0x")) != NULL) || (_tcsstr(pcToken,_T("0X")) != NULL))
							{
                                ...
                                else TRACE_DEBUG(eError,eConfig,_T(__FILE__),_T(__FUNCTION__),__LINE__,_T("Error: %S"),_T("les references doivent etre du type eTYPE_INT8 dans une trame JBUS1_2"));
							}
							else
							{
								pElem = new CElemInt8();
...
LPTSTR CElemTrameJbusReadBitsRP_1_2::szGetConfig(LPTSTR pszText, int iSizeMax)
{
	...
	iSizeMax -= lstrlen(pszText);
	// nbr byte
    _stprintf(szTemp, sizeof(szTemp)/sizeof(TCHAR),_T("%d"),m_NbrBytes.ucGetVal());
	lstrcat(pszText,szTemp);
	// adresse bit
	for (i = 0; i < m_ListParams.iGetCount();i++)
	{
		...
		for (j = 0; pElemFieldBit8 && (j < 8); j++)
		{
			...
			if (pElem)
			{
				...
				{
                    _stprintf(szTemp, sizeof(szTemp)/sizeof(TCHAR),_T("%d"),pElem->ucGetVal());
				}
				else
				{
                    _stprintf(szTemp, sizeof(szTemp)/sizeof(TCHAR),_T("0x%08x"),pElem->iGetType());
				}
				lstrcat(pszText,szTemp);
			}
			else
...
BOOL CElemTrameJbusReadWordsRQ_3_4::bSetConfig(LPTSTR pszText,int &iCurrentPos,CListStream *pListStream)
{
    ...
    _stprintf(szText, sizeof(szText)/sizeof(TCHAR),_T("CElemTrameJbusReadWordsRQ_3_4 : %S"),pszText);
	SetLabel(szText);
	if (bReturn = CElemTrameJbus::bSetConfig(pszText,iCurrentPos,pListStream))
	{
		pszText += iCurrentPos;
...
LPTSTR CElemTrameJbusReadWordsRQ_3_4::szGetConfig(LPTSTR pszText, int iSizeMax)
{
	...
	lstrcat(pszText,szSeps);
	// nbr words
    _stprintf(szTemp, sizeof(szTemp)/sizeof(TCHAR),_T("%d"),m_NbrWords.nGetVal());
	lstrcat(pszText,szTemp);
...
BOOL CElemTrameJbusReadWordsRP_3_4::bSetConfig(LPTSTR pszText,int &iCurrentPos,CListStream *pListStream)
{
    ...
    _stprintf(szText, sizeof(szText)/sizeof(TCHAR),_T("CElemTrameJbusReadWordsRP_3_4 : %S"),pszText);
	SetLabel(szText);
	if (bReturn = CElemTrameJbus::bSetConfig(pszText,iCurrentPos,pListStream))
	{
		pszText += iCurrentPos;
...
LPTSTR CElemTrameJbusReadWordsRP_3_4::szGetConfig(LPTSTR pszText, int iSizeMax)
{
	...
	for (i = 0; i < m_ListParams.iGetCount();i++)
	{
		...
		if (pElem)
		{
			if (NUM_ID(pElem->iGetType()) == 0x000000FF)
			{
                _stprintf(szTemp, sizeof(szTemp)/sizeof(TCHAR),_T("%d"),((CElemInt16*)pElem)->nGetVal());
			}
			else
			{
                _stprintf(szTemp, sizeof(szTemp)/sizeof(TCHAR),_T("0x%08x"),pElem->iGetType());
...
BOOL CElemTrameJbusWriteBit_5::bSetConfig(LPTSTR pszText,int &iCurrentPos,CListStream *pListStream)
{
    ...
    _stprintf(szText, sizeof(szText)/sizeof(TCHAR),_T("CElemTrameJbusWriteBit_5 : %S"),pszText);
	SetLabel(szText);
	if (bReturn = CElemTrameJbus::bSetConfig(pszText,iCurrentPos,pListStream))
	{
		pszText += iCurrentPos;
...
LPTSTR CElemTrameJbusWriteBit_5::szGetConfig(LPTSTR pszText, int iSizeMax)
{
	...
	if (pElem)
	{
		if (NUM_ID(pElem->iGetType()) == 0x000000FF)
		{
            _stprintf(szTemp, sizeof(szTemp)/sizeof(TCHAR),_T("%d"),((CElemInt8*)pElem)->ucGetVal());
		}
		else
		{
            _stprintf(szTemp, sizeof(szTemp)/sizeof(TCHAR),_T("0x%08x"),pElem->iGetType());
...
BOOL CElemTrameJbusWriteWord_6::bSetConfig(LPTSTR pszText,int &iCurrentPos,CListStream *pListStream)
{
    ...
    _stprintf(szText, sizeof(szText)/sizeof(TCHAR),_T("CElemTrameJbusWriteWord_6 : %S"),pszText);
	SetLabel(szText);
	if (bReturn = CElemTrameJbus::bSetConfig(pszText,iCurrentPos,pListStream))
	{
		pszText += iCurrentPos;
...
BOOL CElemTrameJbusWriteBitsRQ_15::bSetConfig(LPTSTR pszText,int &iCurrentPos,CListStream *pListStream)
{
    ...
    _stprintf(szText, sizeof(szText)/sizeof(TCHAR),_T("CElemTrameJbusWriteBitsRQ_15 : %S"),pszText);
	SetLabel(szText);
	if (bReturn = CElemTrameJbus::bSetConfig(pszText,iCurrentPos,pListStream))
	{
		pszText += iCurrentPos;
...
BOOL CElemTrameJbusWriteBitsRP_15::bSetConfig(LPTSTR pszText,int &iCurrentPos,CListStream *pListStream)
{
    ...
    _stprintf(szText, sizeof(szText)/sizeof(TCHAR),_T("CElemTrameJbusWriteBitsRP_15 : %S"),pszText);
	SetLabel(szText);
	if (bReturn = CElemTrameJbus::bSetConfig(pszText,iCurrentPos,pListStream))
	{
		pszText += iCurrentPos;
...
BOOL CElemTrameJbusWriteWordsRQ_16::bSetConfig(LPTSTR pszText,int &iCurrentPos,CListStream *pListStream)
{
    ...
    _stprintf(szText, sizeof(szText)/sizeof(TCHAR),_T("CElemTrameJbusWriteWordsRQ_16 : %S"),pszText);
	SetLabel(szText);
	if (bReturn = CElemTrameJbus::bSetConfig(pszText,iCurrentPos,pListStream))
	{
		pszText += iCurrentPos;
...
BOOL CElemTrameJbusWriteWordsRP_16::bSetConfig(LPTSTR pszText,int &iCurrentPos,CListStream *pListStream)
{
    ...
    _stprintf(szText, sizeof(szText)/sizeof(TCHAR),_T("CElemTrameJbusWriteWordsRP_16 : %S"),pszText);
	SetLabel(szText);
	if (bReturn = CElemTrameJbus::bSetConfig(pszText,iCurrentPos,pListStream))
	{
		pszText += iCurrentPos;
...
BOOL CElemTrameJbusSocket_20::bSetConfig(LPTSTR pszText,int &iCurrentPos,CListStream *pListStream)
{
    ...
    _stprintf(szText, sizeof(szText)/sizeof(TCHAR),_T("CElemTrameJbusSocket_20 : %S"),pszText);
	SetLabel(szText);
	if (bReturn = CElemTrameJbus::bSetConfig(pszText,iCurrentPos,pListStream))
	{
		pszText += iCurrentPos;
...
LPTSTR CElemTrameJbusSocket_20::szGetConfig(LPTSTR pszText, int iSizeMax)
{
	...
	iSizeMax -= lstrlen(pszText);
	// nbr m_NbrParams
    _stprintf(szTemp, sizeof(szTemp)/sizeof(TCHAR),_T("%d"),m_NbrParams.nGetVal());
	lstrcat(pszText,szTemp);
	lstrcat(pszText,szSeps);
	...
	for (i = 0; i < m_ListParams.iGetCount();i++)
	{
		...
		if (pElem)
		{
			if (NUM_ID(pElem->iGetType()) == 0x000000FF)
			{
                _stprintf(szTemp, sizeof(szTemp)/sizeof(TCHAR),_T("0x%08f"),((CElemFloat*)pElem)->fGetVal());
			}
			else
			{
                _stprintf(szTemp, sizeof(szTemp)/sizeof(TCHAR),_T("0x%08x"),pElem->iGetType());
