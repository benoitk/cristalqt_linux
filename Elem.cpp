...
LPTSTR CElemBase::szGetConfig(LPTSTR pszText, int iSizeMax)
{
	...
	// ID
//    _stprintf(pszText, sizeof(pszText)/sizeof(TCHAR),_T("0x%08x"),iGetType());
    _stprintf(pszText, iSizeMax,_T("0x%08x"),iGetType());
...
LPTSTR CElemFieldBit8::szGetConfig(LPTSTR pszText, int iSizeMax)
{
	...
	for (i = 0; (i < 8); i++)
	{
		...
		if (pElem)
		{
			...
			{
//                _stprintf(szTemp, sizeof(pszText)/sizeof(TCHAR),_T("%d"),pElem->ucGetVal());
                _stprintf(szTemp, iSizeMax,_T("%d"),pElem->ucGetVal());
            }
			else
			{
                _stprintf(szTemp, iSizeMax,_T("0x%08x"),pElem->iGetType());
			}
			lstrcat(pszText,szTemp);
		}
		else
...
LPTSTR CElemNbr::szGetConfig(LPTSTR pszText, int iSizeMax)
{
	...
	// ID
//    _stprintf(pszText, sizeof(pszText)/sizeof(TCHAR),_T("0x%08x"),iGetType());
    //DANGER, si sizeMax est plus grand que la taille de pszText
    _stprintf(pszText, iSizeMax,_T("0x%08x"),iGetType());

    qDebug() << "iGetType()" << iGetType()  << " iSizeMax " << iSizeMax << " pszText :";
    _tprintf(pszText);
