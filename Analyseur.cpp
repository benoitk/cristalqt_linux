		...
		for (m_indexSeqence = 0;m_bRun && m_CmdRun.ucGetVal() && (m_indexSeqence < NBR_SEQUENCE_CYCLE_MAX); ++m_indexSeqence)
		{
			...
			else 
			{
				...
				if (pStream && pDuree && (pNumVoie->ucGetVal() < m_iNbrStream))
				{
					...
					{
                        _stprintf(szTrace, sizeof(szTrace)/sizeof(TCHAR),_T("Check Calib : sequence: %d , voie : %d, duree : %d \n")
								, m_indexSeqence
								, pNumVoie->ucGetVal()
								, pDuree->nGetVal());
                        ...
                        _stprintf(szTrace, sizeof(szTrace)/sizeof(TCHAR),_T("Valeur de la boucle : cmdRun: %d , m_CmdJumpStep : %d, m_CounterCycle : %d , m_iTimeElapse : %d, duree : %d \n")
								, m_CmdRun.ucGetVal() 
