 ...
 *****************************************************************************/
BOOL CSerialPort::bReadConfig(QString pszFileName)
{
	TCHAR szPort[ 64 ];
	long lSizeDataMax;
	BOOL bReturn;
	...
	memset(&m_DCB,0,sizeof(m_DCB));
#ifdef _WIN32_WCE
    _stprintf(szPort, sizeof(szPort)/sizeof(TCHAR), _T("COM%d:"), m_nNumPort );
#else
    _stprintf(szPort, sizeof(szPort)/sizeof(TCHAR), _T("\\\\.\\COM%d"), m_nNumPort );
#endif
 ...
 * @parm :	LPCSTR | szIni | fichier de configuration
 *****************************************************************************/
BOOL CSerialPort::bWriteConfig(QString pszFileName)
{
	BOOL bReturn;
...
//#ifdef _WIN32_WCE
//    __stprintf(szPort, sizeof(szPort)/sizeof(TCHAR), _T("COM%d:"), m_nNumPort );
//#else
//    __stprintf(szPort, sizeof(szPort)/sizeof(TCHAR), _T("\\\\.\\COM%d"), m_nNumPort );
//#endif
//	m_hTTY = CreateFile( szPort, GENERIC_READ | GENERIC_WRITE,0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_SYSTEM/*FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OVERLAPPED*/, NULL );
...
BOOL CSerialPort::bAddExchangeJbus(LPTSTR pszRQ,LPTSTR pszRP,CListStream *pListStream)
{
	...
	{
        _stprintf(szText, sizeof(szText)/sizeof(TCHAR),_T("Requete perif : %d"), m_pListExchange->iGetCount()+1);
