...
{
    _tprintf(_T(" ReadConfigMsgError(LPTSTR szFileMsg) \n"));
	memset(&_aszErrorText[0][0],0,sizeof(_aszErrorText));
    _tprintf(_T(" memset(&_aszErrorText[0][0],0,sizeof(_aszErrorText)); \n"));
    dwGetPrivateProfileString(_T("MSG"), _T("eErrorFindRqAndExecuteFromIHM"),_T("Parametre ou variable inconnu"),_aszErrorText[eErrorFindRqAndExecuteFromIHM],MAX_PATH, szFileMsg);
    _tprintf(_T(" dwGetPrivateProfileString %S \n"), _aszErrorText[eErrorFindRqAndExecuteFromIHM]);
    ...
    dwGetPrivateProfileString(_T("MSG"), _T("eErrorCycleWriteConfig"),_T("Erreur d ecriture de la configuration du cycle"),_aszErrorText[eErrorCycleWriteConfig],MAX_PATH,szFileMsg);
    dwGetPrivateProfileString(_T("MSG"), _T("eErrorCycleExecute"),_T("Erreur d'execution du cycle"),_aszErrorText[eErrorCycleExecute],MAX_PATH,szFileMsg);
    ...
    dwGetPrivateProfileString(_T("MSG"), _T("eErrorCycleZeroWriteConfig"),_T("Erreur d'ecriture de la configuration du cycle de zero"),_aszErrorText[eErrorCycleZeroWriteConfig],MAX_PATH,szFileMsg);
    dwGetPrivateProfileString(_T("MSG"), _T("eErrorCycleZeroExecute"),_T("Erreur d'execution du cycle de zero"),_aszErrorText[eErrorCycleZeroExecute],MAX_PATH,szFileMsg);
    ...
    dwGetPrivateProfileString(_T("MSG"), _T("eErrorCycleCalibWriteConfig"),_T("Erreur d'ecriture de la configuration du cycle de calibration"),_aszErrorText[eErrorCycleCalibWriteConfig],MAX_PATH,szFileMsg);
    dwGetPrivateProfileString(_T("MSG"), _T("eErrorCycleCalibExecute"),_T("Erreur d'execution du cycle de calibration"),_aszErrorText[eErrorCycleCalibExecute],MAX_PATH,szFileMsg);
    ...
    dwGetPrivateProfileString(_T("MSG"), _T("eErrorCycleCleanupWriteConfig"),_T("Erreur d'ecriture de la configuration du cycle de nettoyage"),_aszErrorText[eErrorCycleCleanupWriteConfig],MAX_PATH,szFileMsg);
    dwGetPrivateProfileString(_T("MSG"), _T("eErrorCycleCleanupExecute"),_T("Erreur d'execution du cycle de nettoyage"),_aszErrorText[eErrorCycleCleanupExecute],MAX_PATH,szFileMsg);
    ...
    dwGetPrivateProfileString(_T("MSG"), _T("eErrorSocketWriteConfig"),_T("Erreur d'ecriture de la configuration de la socket"),_aszErrorText[eErrorSocketWriteConfig],MAX_PATH,szFileMsg);
    dwGetPrivateProfileString(_T("MSG"), _T("eErrorSocketRunThread"),_T("Erreur d'execution de la socket"),_aszErrorText[eErrorSocketRunThread],MAX_PATH,szFileMsg);
    ...
    dwGetPrivateProfileString(_T("MSG"), _T("eErrorAppWriteConfig"),_T("Erreur d'ecriture de la configuration de l'application"),_aszErrorText[eErrorAppWriteConfig],MAX_PATH,szFileMsg);
    dwGetPrivateProfileString(_T("MSG"), _T("eErrorAppRunThread"),_T("Erreur d'execution de l'application"),_aszErrorText[eErrorAppRunThread],MAX_PATH,szFileMsg);
    ...
    dwGetPrivateProfileString(_T("MSG"), _T("eErrorCarteIOWriteConfig"),_T("Erreur d'ecriture sur la carte IO"),_aszErrorText[eErrorCarteIOWriteConfig],MAX_PATH,szFileMsg);
    dwGetPrivateProfileString(_T("MSG"), _T("eErrorCarteIOExecute"),_T("Erreur d'execution sur la carte IO"),_aszErrorText[eErrorCarteIOExecute],MAX_PATH,szFileMsg);
    ...
    dwGetPrivateProfileString(_T("MSG"), _T("eErrorCarteJBusSlavebWriteConfig"),_T("Erreur d'ecriture de la configuration de la carte JBUS esclave"),_aszErrorText[eErrorCarteJBusSlavebWriteConfig],MAX_PATH,szFileMsg);
    ...
    dwGetPrivateProfileString(_T("MSG"), _T("eErrorCarteMesureWriteConfig"),_T("Erreur d'ecriture de la configuration de la carte mesure"),_aszErrorText[eErrorCarteMesureWriteConfig],MAX_PATH,szFileMsg);

    dwGetPrivateProfileString(_T("MSG"), _T("eErrorMesureStatusThreshold1"),_T("Erreur depassement seuil 1"),_aszErrorText[eErrorMesureStatusThreshold1],MAX_PATH,szFileMsg);
    dwGetPrivateProfileString(_T("MSG"), _T("eErrorMesureStatusThreshold2"),_T("Erreur depassement seuil 2"),_aszErrorText[eErrorMesureStatusThreshold2],MAX_PATH,szFileMsg);
    dwGetPrivateProfileString(_T("MSG"), _T("eErrorMesureValMax"),_T("Erreur depassement valeur max"),_aszErrorText[eErrorMesureValMax],MAX_PATH,szFileMsg);
    dwGetPrivateProfileString(_T("MSG"), _T("eErrorMesureValMin"),_T("Erreur depassement valeur min"),_aszErrorText[eErrorMesureValMin],MAX_PATH,szFileMsg);
    ...
    _tprintf(_T("FIN  ReadConfigMsgError(LPTSTR szFileMsg) \n"));
}
...
//	SYSTEMTIME  stTime;
	...
	{
//		GetLocalTime(&stTime);
        _stprintf(szTextDebug, sizeof(szTextDebug)/sizeof(TCHAR), _T(" %S\r\n"),format);
    
		va_start(arglist, format);
        _vstprintf(szTextInfo, sizeof(szTextInfo)/sizeof(TCHAR), szTextDebug, arglist);
//        _stprintf(szTextDebug, sizeof(szTextDebug)/sizeof(TCHAR), _T("File: %S ; Function : %S ; Line :%d LVL:%d ; ORIG:%d ; %02d:%02d:%02d:%04d ; ==> "),szFile,szFunction,noLigne, eTypeMsg,eOrigineMsg,timeinfo->tm_hour,timeinfo->tm_minute,timeinfo->tm_sec,stTime.wMilliseconds);
...
//			OutputDebugString(szTextInfo);
		...
		if (_bLogFile)
		{
            ...
            //_stprintf(szLogFile, _T("%S\\%04d_%02d_%02d.log"),_szLogFileDir,timeinfo->tm_year,timeinfo->tm_month,timeinfo->tm_wday);
            /*_stprintf(szLogFile, _T("%04d_%02d_%02d.log"),timeinfo->tm_year,timeinfo->tm_month,timeinfo->tm_wday);
		...
		if (_bErrFile && (eTypeMsg == eError))
		{
			...
			bEcrireFichierLog(szTextInfo, _szLogErrorPrgFileDir, szFileName);
			/*EnterCriticalSection(&_LogCriticalSection);
            //_stprintf(szLogFile,_T("%S\\%04d_%02d_%02d.err"),_szLogFileDir,timeinfo->tm_year,timeinfo->tm_month,timeinfo->tm_wday);
            _stprintf(szLogFile,_T("%04d_%02d_%02d.err"),timeinfo->tm_year,timeinfo->tm_month,timeinfo->tm_wday);
			hFile = CreateFile(szGetFullPathName(szLogFile,szText),GENERIC_READ|GENERIC_WRITE,0,NULL,OPEN_ALWAYS,FILE_ATTRIBUTE_NORMAL,NULL);
			if (hFile != INVALID_HANDLE_VALUE)
			{
				SetFilePointer(hFile,0,NULL,FILE_END);
...
//			     , timeinfo->tm_year,timeinfo->tm_month,timeinfo->tm_wday,timeinfo->tm_hour,timeinfo->tm_minute,timeinfo->tm_sec
...
//	GetLocalTime(&stTime);
    ...
    l  =  _stprintf(szMessage, sizeof(szMessage)/sizeof(TCHAR)
                   , _T( "%04d/%02d/%02d ; %02d:%02d:%02d ; %S=%-d ")
                   , timeinfo->tm_year,timeinfo->tm_mon,timeinfo->tm_wday,timeinfo->tm_hour
                   , timeinfo->tm_min,timeinfo->tm_sec
				   , argNumCurrentStream->szGetLabel()
				   , (argNumCurrentStream->ucGetVal()+1));
    i  =  _stprintf(szUserMessage, sizeof(szUserMessage)/sizeof(TCHAR)
                   , _T( "%04d/%02d/%02d ; %02d:%02d:%02d ; %S=%-d ")
                   , timeinfo->tm_year,timeinfo->tm_mon,timeinfo->tm_wday,timeinfo->tm_hour
                   , timeinfo->tm_min,timeinfo->tm_sec
				   , argNumCurrentStream->szGetLabel()
	...
	for(int k=0; (k < argObjVoie->iGetNbrMesure()) && (k < NBR_MESURE_MAX); k++)
	{
		...
		if( !(argObjVoie->pGetAt(k)->m_iCompteurMesure < NB_MESURE_MOYENNEE))
		{
            ...
            l  += _stprintf(szMessage, sizeof(szMessage)/sizeof(TCHAR) + l
            ...
            i  += _stprintf(szUserMessage, sizeof(szUserMessage)/sizeof(TCHAR) + i
                           , _T( "; %S=%-f; ")
				     	   , argObjVoie->pGetAt(k)->m_Val.szGetLabel()
						   , argObjVoie->pGetAt(k)->m_SommeMesure.fGetVal());
	...
	if(bWriteFile)
	{
        _stprintf(szMessage, sizeof(szMessage)/sizeof(TCHAR) + l, _T( "\r\n"));
        _stprintf(szUserMessage, sizeof(szUserMessage)/sizeof(TCHAR) + i, _T( "\r\n"));
...
//	GetLocalTime(&stTime);
    ...
    time_t rawtime;
    const WCHAR* weekday[] = {_T("Dimanche"), _T("Lundi"),_T("Mardi"), _T("Mercredi"), _T("Jeudi"),_T("vendredi"),_T("samedi")};
    time(&rawtime);
    ...
    _stprintf(szMessageScreen, sizeof(szMessageScreen)/sizeof(TCHAR)
                   , _T( "%S \r\n")
				   , argpszMessage);
...
//    int dwTick = GetTickCount();
	int l;
    l  =  _stprintf(szMessage, sizeof(szMessage)/sizeof(TCHAR)
        , _T( "%04d/%02d/%02d ; %02d:%02d:%02d; ID:%d ; %S \r\n")
                   , timeinfo->tm_year,timeinfo->tm_mon,timeinfo->tm_wday,timeinfo->tm_hour
                   , timeinfo->tm_min,timeinfo->tm_sec
                   , iID
				   , argpszMessage);
...
void verifierDefaut(BYTE argcErrorToCheck, CStream* argObjVoie, int argiNumMesure, int argiNumPas, WCHAR* argpszMessage, int* argpIndexMessage, int argeError, CElemInt8* argNumCurrentStream)
{
	...
	{
        *argpIndexMessage  += _stprintf(argpszMessage + *argpIndexMessage, sizeof(argpszMessage + *argpIndexMessage)/sizeof(TCHAR)
                                       , _T( "; %S; Step %d; %S ")
...
//	GetLocalTime(&stTime);
    ...
    timeinfo = localtime(&rawtime);
    mktime(timeinfo);
    _stprintf(argszFileName, MAX_PATH
			 , _T( "%04d_%02d_%02d")
             , timeinfo->tm_year,timeinfo->tm_mon,timeinfo->tm_wday,timeinfo->tm_hour);
