...
CWinMainView::CWinMainView(CWinMainControler* argoControler, QWidget *parent, Qt::WindowFlags flags)
...
{
    qDebug() << "#### CWinMainView::init" << endl;

	m_tabWidgetCentral = new QTabWidget(); //Pas de parent, setCentralWidget l'attribura Ã  la fenÃªtre
    ...
    qDebug() << "#### CWinMainView::init 0" << endl;
	...
	for(int i=0; i<m_pModel->getNbStream(); ++i)
	{
        ...
        qDebug() << "#### CWinMainView::init 1 " << i <<endl;
		...
		{
            qDebug() << "#### CWinMainView::init 2 " << j <<endl;
...
#ifdef MULTI_MEASURE
            if(j==0) //Afficher le bouton de dÃ©tail uniquement pour la premiÃ¨re mesure.
            {
...
#ifdef MAGNESIUM
			++j; //ne pas afficher la deuxiÃ¨me mesure
#endif      
...
//	m_tabWidgetCentral->setLayout(mainLayout);
	...
	//Utilisation des boutons pour l'affichage seul (pour ne pas refaire le CSS), ils ne sont pas connectÃ©s
...
#ifdef SONDE
	gridLayout->addWidget(new QLabel(tr("Mesure Ã©lectrode")), row,0);
#else
...
#ifndef COULEUR
	gridLayout->addWidget(new QLabel(tr("TempÃ©rature Cuve")), row,0);
	gridLayout->addWidget(m_btTemperatureCuve, row, 1);
	gridLayout->addWidget(new QLabel(tr("Â°C")), row++,2);
#endif
	...
	//: Nom de l'onglet Ã  laisser en maj pour toutes les trads
	...
	QLabel* lblMeasureCard = new QLabel(tr("Test Ã©lectrique"));
	...
	QLabel* lblParameter = new QLabel("PramÃ¨tres");
	...
	/*m_btSequenceur = new QPushButton();
	m_btSequenceur->setObjectName("btSequenceur");
	QLabel* lblSequenceur = new QLabel(tr("SÃ©quenceur"));
	lblSequenceur->setObjectName("lblOutils");*/
	m_btSave = new QPushButton("");
	m_btSave->setObjectName("btSave");
	...
	m_btParameter = new QPushButton();
	m_btParameter->setObjectName("btParameter");
	QLabel* lblParameter = new QLabel(tr("ParamÃ¨tres"));
	lblParameter->setObjectName("lblOutils");
	m_btExplorer = new QPushButton();
	m_btExplorer->setObjectName("btExplorer");
...
void CWinMainView::dataUpdate()
{
   ...
   // m_mesureLayout->setColumnMinimumWidth(0, m_mesureLayout->cellRect(0, 0).width()  );
	...
	if(m_pModel->getStatusWaterFailure(m_pModel->nGetCurrentStream()))
		m_lblStatusWaterFailure->setText(tr("DÃ©faut d'eau"));
	else
		m_lblStatusWaterFailure->setText("");
		
	if(m_pModel->getStatusSeuil(m_pModel->nGetCurrentStream()))
		m_lblStatusSeuil->setText(tr("Seuil dÃ©passÃ©"));
	else
	...
	//Trouver la premiÃ¨re voie prÃ©cÃ©dente active
    ...
    {
        m_lblCurrentStream->setText(QString(tr("DÃ©compte ")));
    ...
    m_lblCurrentStep->setText(QString(tr("Pas nÂ°:")) + m_pModel->getCurrentStep());
    ...
    if(m_tabWidgetCentral->currentIndex()==1)
    {
        ...
        m_lblCurrentStepDiag->setText(QString(tr("Pas nÂ°:")) + m_pModel->getCurrentStep());
...
void CWinMainView::addMeasureToGraph(int arg_numStream, float arg_fMesure, const QString& arg_sMesureUnit)
{
	if(m_pModel->getFlagEndCycle())
	{
		...
		float heightActuel;*/
		float a;
		//Pour les Ã©chelle dynamique
		float concentrationMax; //aucun rapport avec les coordonnÃ©e, sert Ã  calculer yActuel de la derniÃ¨re mesure
		float fCorrectionY=1;
		
		////(a dÃ©comenter pour une echelle qui s'ajuste en auto)calcule de concentrationMax 
		//a = m_tabMesureMax[0];
		...
		concentrationMax = m_pModel->getBoutPlageMax(arg_numStream); //(Ã  commenter pour repasser Ã§=Ã  une echel ajustÃ© en auto)
