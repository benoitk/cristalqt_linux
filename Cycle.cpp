...
BOOL CElemCycleStep::bReadConfigExchangeJbus(LPTSTR pszRub, LPTSTR pszKeyRQ,LPTSTR pszKeyRP,QString pszFileName,CElemList *pListExchange,CListStream *pListStream)
{
//    qDebug() << "CElemCycleStep::bReadConfigExchangeJbus";
	BOOL bReturn = FALSE;
	...
	{
        qDebug() << "if (pExchangeJbus = new CElemExchangeJbus())";
        _stprintf(szText, sizeof(szText)/sizeof(TCHAR),_T("CElemCycleStep : (PAS=%S) -> %S - %S"),pszRub, pszKeyRQ,pszKeyRP);
	...
	if (!bReturn) 
	{
        qDebug() << "Erreur dans CElemCycleStep::bReadConfigExchangeJbus Cycle.cpp line 113";
        qDebug() << "rub " << QString::fromWCharArray(pszRub);

        TRACE_DEBUG(eDebug,eConfig,_T(__FILE__),_T(__FUNCTION__),__LINE__,_T("Error: %S "),szGetLabel());
	}

//	delete [] szText;
//    qDebug() << "FIN CElemCycleStep::bReadConfigExchangeJbus";
	return bReturn;
}


BOOL CElemCycleStep::bReadConfig(LPTSTR pszRub, QString pszFileName,CListStream *pListStream)
{
    qDebug() << "CElemCycleStep::bReadConfig";
	BOOL bReturn = TRUE;
    ...
    if (lstrlen(szText) == 0) _stprintf(szText, sizeof(szText)/sizeof(TCHAR),_T("CElemCycleStep : (PAS=%S)"),pszRub);
	SetLabel(szText);
	// nbr de trame begin
	iNbrTrameBegin = iGetPrivateProfileInt(pszRub,_T("NbrBegin"),0,pszFileName);
	for (i = 0;bReturn && (i < iNbrTrameBegin);i++)
	{
        _stprintf(szKeyRQ, sizeof(szKeyRQ)/sizeof(TCHAR),_T("BeginRQ%d"),i+1);
        _stprintf(szKeyRP, sizeof(szKeyRP)/sizeof(TCHAR),_T("BeginRP%d"),i+1);
	...
	{
        _stprintf(szKeyRQ, sizeof(szKeyRQ)/sizeof(TCHAR),_T("EndRQ%d"),i+1);
        _stprintf(szKeyRP, sizeof(szKeyRP)/sizeof(TCHAR),_T("EndRP%d"),i+1);
	...
	{
        _stprintf(szKeyRQ, sizeof(szKeyRQ)/sizeof(TCHAR),_T("RealTimeRQ%d"),i+1);
        _stprintf(szKeyRP, sizeof(szKeyRP)/sizeof(TCHAR),_T("RealTimeRP%d"),i+1);
    ...
    if (!bReturn) TRACE_DEBUG(eDebug,eConfig,_T(__FILE__),_T(__FUNCTION__),__LINE__,_T("Error: %S"),szGetLabel());
    qDebug() << "FIN CElemCycleStep::bReadConfig";
    return (bReturn && (iNbrTrameRealTime>0 || iNbrTrameEnd>0 || iNbrTrameBegin>0));
}

BOOL CElemCycleStep::bWriteConfigExchangeJbus(LPTSTR pszRub, LPTSTR pszKeyRQ,LPTSTR pszKeyRP, int iPos ,QString pszFileName,CElemList *pListExchange)
{
	BOOL bReturn = FALSE;
	...
	{
		// RQ
        _stprintf(szKey, sizeof(szKey)/sizeof(TCHAR),_T("%S%d"),pszKeyRQ,iPos+1);
		...
		// RP
        _stprintf(szKey, sizeof(szKey)/sizeof(TCHAR),_T("%S%d"),pszKeyRP,iPos+1);
...
BOOL CElemCycleStep::bWriteConfig(LPTSTR pszRub,QString pszFileName)
{
	BOOL bReturn = FALSE;
    ...
    if (pszRub == NULL) _stprintf(szRub, sizeof(szRub)/sizeof(TCHAR),_T("%d"),m_Step.nGetVal());
							...
							if (iSizeToWrite < sizeof(aBuffer))
							{
								if (bReturn = pElem->m_pTrameJbusRQ->bSerialize(Context))
								{
									...
									if (iSizeToRead < Context.m_iSize)
									{
                                        ...
                                        QThread::msleep(DW_DELAI_INTER_TRAME);// delai inter-trame
...
BOOL CElemCycle::bReadConfig(QString pszFileName,CListStream *pListStream)
{
    qDebug() << "CElemCycle::bReadConfig";
	BOOL bReturn = FALSE;
	TCHAR szRub[MAX_PATH];
	TCHAR szText[MAX_PATH];
	int i;
	...
	{
        _stprintf(szRub, sizeof(szRub)/sizeof(TCHAR),_T("%d"),i);
    ...
    qDebug() << "FIN CElemCycle::bReadConfig";
	return bReturn;
}

BOOL CElemCycle::bWriteConfig(QString pszFileName)
{
	BOOL bReturn = FALSE;
	int i;
	int iSize;
...
BOOL CElemCycleZero::bReadConfig(QString pszFileName,CListStream *pListStream)
{
	BOOL bReturn = FALSE;
...
BOOL CElemCycleZero::bWriteConfig(QString pszFileName)
{
	BOOL bReturn = FALSE;
...
BOOL CElemCycleCalib::bReadConfig(QString pszFileName,CListStream *pListStream)
{
	BOOL bReturn = FALSE;
...
BOOL CElemCycleCalib::bWriteConfig(QString pszFileName)
{
	BOOL bReturn = FALSE;
...
BOOL CElemCycleCleanup::bReadConfig(QString pszFileName,CListStream *pListStream)
{
	BOOL bReturn = FALSE;
...
BOOL CElemCycleCleanup::bWriteConfig(QString pszFileName)
{
	BOOL bReturn = FALSE;
