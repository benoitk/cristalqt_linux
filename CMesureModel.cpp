	...
	if(m_mapIdRefVar.contains(lIdVar))
	{
		switch(iTypeVar)
		{
			...
			case eTYPE_INT16:
				...
				break;
			case eTYPE_FLOAT:
                QString sPrecision = QString::fromWCharArray(((CElemFloat*)m_mapIdRefVar.value(lIdVar))->szGetConfig(szText, 250));
...
QString CMesureModel::getVarMesureLbl(QString argsObjectName)
{
	...
	long lIdVar = argsObjectName.toLong(&convertionOk, 0); 
	if(m_mapIdRefVar.contains(lIdVar))
        return QString::fromWCharArray(m_mapIdRefVar.value(lIdVar)->szGetLabel()
...
QString CMesureModel::getVarMesureUnit(QString argsObjectName)
{
	...
	if(m_mapIdRefVar.contains(lIdVar))
        return QString::fromWCharArray(m_mapIdRefVar.value(lIdVar)->m_szUnit.szGetLabel()
...
void CMesureModel::writeElemConfigIni(WCHAR* arg_szSection, WCHAR* arg_szElemName, CElemBase* arg_pCElemBase)
{
    ...
    _stprintf(szText, sizeof(szText)/sizeof(TCHAR),_T("config.ini"));
    szGetFullPathName(szText);
